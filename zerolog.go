	...
	return func(next echo.HandlerFunc) echo.HandlerFunc {
		return func(ctx echo.Context) (err error) {
			...
			for k, v := range cfg.FieldMap {
				...
				case logLatency:
					l := stop.Sub(start)
					entry = entry.Str(k, strconv.FormatInt(int64(l), base))
				...
				case logBytesOut:
					entry = entry.Str(k, strconv.FormatInt(res.Size, base))
				default:
					switch {
					case strings.HasPrefix(v, logHeader+":"):
						entry = entry.Str(k, ctx.Request().Header.Get(v[8:]))
					case strings.HasPrefix(v, logQuery+":"):
						entry = entry.Str(k, ctx.QueryParam(v[7:]))
					case strings.HasPrefix(v, logForm+":"):
						entry = entry.Str(k, ctx.FormValue(v[6:]))
					case strings.HasPrefix(v, logCookie+":"):
